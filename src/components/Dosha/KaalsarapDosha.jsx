...
const KaalsarapDosha = ({ userData }) => {
        ...
        const kalsarapdosh = async () => {
            try {
                ...
                const data = await fetch(
                    `${API_BASE}/astro/dosha/kaalsarp-dosh?date=${formattedDate}&time=${time}&latitude=${latitude}&longitude=${longitude}&tz=5.5&lang=${i18n.language === 'hi' ? 'hi' : 'en'}`,
                    {
                        headers: {
    ...
    return (
        <div>
            <div className="bg-[#1A2742] rounded-xl border border-amber-400 p-6">
                ...
                <div className="grid md:grid-cols-2 gap-5">
                    ...
                    <div className="bg-[#243454] rounded-lg p-4">
                        ...
                        <p className={`text-xl font-bold ${kalsarap.is_dosha_present
                            ? "text-red-400"
                            : "text-green-400"
                            }`}>
            ...
            {!isPremium && (
                <div className="mt-6">
                    <PremiumLock title={`${t("kaalsarpDoshReport")} की पूरी जानकारी के लिए`} />
                </div>
            )}
