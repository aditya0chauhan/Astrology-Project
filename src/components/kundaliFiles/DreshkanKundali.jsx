...
const DreshkanKundali = ({ userData }) => {
  const [dreshkanChart, setDreshkanChart] = useState(null)
  ...
  useEffect(() => {
    ...
    const key1 = `${formattedDate}_${time}_${latitude}_${longitude}_d7`
    if (svgCache.has(key)) {
      setDreshkanChart(svgCache.get(key))
      return
    }
    if (svgCache.has(key)) {
      setD7Chart(svgCache.get(key1))
      return
    }
    ...
    const fetchChart = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          if (!cancelled) {
            setDreshkanChart(clean)
            svgCache.set(key, clean)
          }
        }
        const saptres = await fetch(
          `${API_BASE}/astro/chart_image/d7?date=${formattedDate}&time=${time}&latitude=${latitude}&longitude=${longitude}&tz=5.5&style=north&lang=hi&colored_planets=true`)
        if (!saptres.ok) {
          console.warn('Dreshkan chart fetch failed:', res.status, res.statusText)
        } else {
          const svg = await saptres.text()
          const clean = normalizeSvg(svg)
          if (!cancelled) {
            setD7Chart(clean)
            svgCache.set(key, clean)
      ...
      {dreshkanChart && (
        <div className="mt-10 bg-white lp-6 rounded-xl flex flex-col justify-start items-start">
          ...
          <div className='chart-inner w-full rounded-lg bg-white pr-10 lg:p-4 lg:flex justify-center items-center'>
            <div className="chart-svg-wrapper w-full lg:w-[60%] lg:m-auto" dangerouslySetInnerHTML={{ __html: dreshkanChart }} />
          </div>
        </div>
      )}
        ...
        <div className="mt-10 bg-white lp-6 rounded-xl flex flex-col justify-start items-start">
          ...
          <div className='chart-inner w-full rounded-lg bg-white pr-10 lg:p-4 lg:flex justify-center items-center'>
            <div className="chart-svg-wrapper w-full lg:w-[60%] lg:m-auto" dangerouslySetInnerHTML={{ __html: d7chart }} />
          </div>
        </div>
      )}
    </div>
...
export default DreshkanKundali