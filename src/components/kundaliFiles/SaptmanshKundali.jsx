...
const SaptmanshKundali = ({ userData }) => {
  ...
  useEffect(() => {
    ...
    const key = `${formattedDate}_${time}_${latitude}_${longitude}_d7`
    if (svgCache.has(key)) {
      setD7Chart(svgCache.get(key))
      return
    }
    ...
    const fetchChart = async () => {
      ...
      try {
        ...
        if (!res.ok) {
          console.warn('Saptmansh chart fetch failed:', res.status, res.statusText)
        } else {
...
export default SaptmanshKundali
