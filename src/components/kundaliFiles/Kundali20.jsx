...
const Kundali20 = ({ userData }) => {
  ...
  useEffect(() => {
    ...
    const key = `${formattedDate}_${time}_${latitude}_${longitude}_d20`
    if (svgCache.has(key)) {
      setD20(svgCache.get(key))
      return
    }
    ...
    const fetchChart = async () => {
      ...
      try {
        const res = await fetch(
          `https://api.jyotishamastroapi.com/api/chart_image/d20?date=${formattedDate}&time=${time}&latitude=${latitude}&longitude=${longitude}&tz=5.5&style=north&lang=hi&colored_planets=true`,
          {
        ...
        <div className="mt-10 bg-white lp-6 rounded-xl flex flex-col justify-start items-start">
         ...
         विंशांश कुंडली
          ...
          <div className='chart-inner w-full rounded-lg bg-white pr-10 lg:p-4 lg:flex justify-center items-center'>
            <div className="chart-svg-wrapper w-full lg:w-[60%] lg:m-auto" dangerouslySetInnerHTML={{ __html: d20 }} />
          </div>
        </div>
      )}
    </div>
