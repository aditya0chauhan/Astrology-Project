...
const Kundali30 = ({ userData }) => {
  ...
  useEffect(() => {
    ...
    const key = `${formattedDate}_${time}_${latitude}_${longitude}_d30`
    if (svgCache.has(key)) {
      setD30(svgCache.get(key))
      return
    }
    ...
    const fetchChart = async () => {
      ...
      try {
        const res = await fetch(
          `https://api.jyotishamastroapi.com/api/chart_image/d30?date=${formattedDate}&time=${time}&latitude=${latitude}&longitude=${longitude}&tz=5.5&style=north&lang=hi&colored_planets=true`,
          {
            headers: { key: import.meta.env.VITE_ASTRO_API_KEY },
          }
        )
        if (!res.ok) {
          console.warn('D30 chart fetch failed:', res.status, res.statusText)
        } else {
      ...
      {d30 && (
        <div className="mt-10 bg-white lp-6 rounded-xl flex flex-col justify-start items-start">
          ...
          <div className='chart-inner w-full rounded-lg bg-white pr-10 lg:p-4 lg:flex justify-center items-center'>
            <div className="chart-svg-wrapper w-full lg:w-[60%] lg:m-auto" dangerouslySetInnerHTML={{ __html: d30 }} />
          </div>
        </div>
      )}
    </div>
